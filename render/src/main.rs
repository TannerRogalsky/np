...
#[tokio::main]
async fn main() {
    let game_id = 4725907836895232;
    let ticks = (0..=804).step_by(6);
    // let tick = game.most_recent_tick(&db).await.unwrap();
    // let tick = 6;
    for tick in ticks {
        process(game_id, tick);
    }
}

fn process(game_id: i64, tick: i64) {
    let root = std::path::Path::new(".");
    let raw = std::fs::read(
        root.join("data")
            .join(format!("{}_tick{:0>5}.json", game_id, tick)),
    )
    .unwrap();
    let reports: Vec<api::order::Report> = serde_json::from_slice(&raw).unwrap();
    ...
    svg::save(
        root.join("docs")
            .join("svgs")
            .join(format!("{}_tick{:0>5}.svg", game_id, tick)),
        &document,
    )
    .unwrap();
}
